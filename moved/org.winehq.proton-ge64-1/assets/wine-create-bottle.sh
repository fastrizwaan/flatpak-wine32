...
# URL: https://github.com/fastrizwaan/org.winehq.proton-ge64-1
# ============================================================================ #
...
mkdir -p ~/.local/share/org.winehq.proton-ge64-1/bottles/

myBaseNamePrefix=$(echo $myBaseName|tr ' ' '_');

###############################################
cat << EOF > ~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix.sh
#!/bin/bash
export WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix
...
if [ "\$choice" = "Run Winetricks..." ]; then  
   WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix flatpak run --command=winetricks org.winehq.org.winehq.proton-ge64-1 --gui
...
prog=\$(echo \$step)
	...
	( for i in \${mydlls[*]};
	  do
    	echo \$prog
	    echo "# Installing \$i..."
	    WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix flatpak run --command=winetricks org.winehq.org.winehq.proton-ge64-1 --unattended \$i
   ...
   WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix flatpak run --command=winecfg org.winehq.org.winehq.proton-ge64-1
elif [ "\$choice" = "Open Explorer++..." ]; then
   WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix flatpak run --command=wine org.winehq.org.winehq.proton-ge64-1 /app/explorer++/Explorer++.exe
elif [ "\$choice" = "Open Shell..." ]; then   
 gnome-terminal -- bash -c "flatpak run --command=bash org.winehq.org.winehq.proton-ge64-1"
elif [ "\$choice" = "Delete Bottle: $myBaseNamePrefix" ]; then
rm -rfv ~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix; 
rm -f "$HOME/.local/share/applications/org.winehq.proton-ge64-1/$myBaseName.desktop"
rm -f "$myFile.desktop" 
rm -f "$myFile.icon.png"
rm -f ~/.local/share/org.winehq.proton-ge64-1/bottles/"$myBaseNamePrefix.sh"
elif [ "\$choice" = "Launch $myBaseNamePrefix" ]; then
export base=\$(basename "$myFile")
export dire=\$(dirname "$myFile")
cd "\$dire"
WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix flatpak run --command=wine org.winehq.org.winehq.proton-ge64-1 "\$base"
fi
EOF

###############################################
chmod +x ~/.local/share/org.winehq.proton-ge64-1/bottles/"$myBaseNamePrefix.sh"
chmod +x ~/.local/share/org.winehq.proton-ge64-1/bottles/*.sh
    
    # Generate desktop entry specifications to be added to the application launcher.
    deskEntry="[Desktop Entry]"\\n"Exec=bash -c ~/.local/share/org.winehq.proton-ge64-1/bottles/$myBaseNamePrefix.sh"\\n"Name=\
...
$category;"\\n"Icon="$myFile.icon.png""
    ...
    mkdir -p "$HOME/.local/share/applications/org.winehq.proton-ge64-1/"
    
    ln -sf "$myFile".desktop  $HOME/.local/share/applications/org.winehq.proton-ge64-1/"$myBaseName".desktop
