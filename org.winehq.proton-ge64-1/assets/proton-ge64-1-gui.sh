...
#!/bin/bash
export WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/default
...
mkdir -p ~/.local/share/applications/org.winehq.proton-ge64-1/

# Create kill shortcut for convenience
mkdir -p ~/.local/share/applications/org.winehq.proton-ge64-1/
echo '
[Desktop Entry]
Exec=flatpak kill org.winehq.org.winehq.proton-ge64-1
Name=Proton-6.4-GE-1 kill (6.4)
Type=Application
Categories=Application;;
Icon=org.winehq.org.winehq.proton-ge64-1-kill
Keywords=flatpak; proton; wine; kill;
' > ~/.local/share/applications/org.winehq.proton-ge64-1/killall_proton-ge64-1.desktop


# org.winehq.proton-ge64-1-gui.sh
# for GUI Dialog
if [ $# -eq 0 ];  then
   echo "No arguments supplied; use --help"
 
   choice=$(zenity --title "Proton-GE-64-1 (6.4)" --width=240 --height=300 \
					 --list \
					 --radiolist --column " " \
					 --column "Action" \
							  0 "Run Winetricks..." \
						   ...
						   TRUE "Open Explorer++" \
							  0 "Delete Bottle" \
					 --text "Select Action for ~/.local/share/org.winehq.proton-ge64-1/default " )

   # exit when Cancel is clicked
   [[ -z "$choice" ]] && exit 1
   
	if [ "$choice" = "Run Winetricks..." ]; then  
	   WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/default $WINETRICKS --gui
...
echo $size $step ${mydlls[*]}
	...
	( for i in ${mydlls[*]};
	  do
    	echo $prog
	    echo "# Installing $i..."
	    WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/default $WINETRICKS --unattended  $i
	...
	# winecfg
	   ...
	   WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/default winecfg

	elif [ "$choice" = "Delete Bottle" ]; then
	   rm -rfv ~/.local/share/org.winehq.proton-ge64-1/default; 
       rm ~/.local/share/applications/org.winehq.proton-ge64-1/killall_proton-ge64-1.desktop

	elif [ "$choice" = "Open Explorer++" ]; then
	   WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/default $WINEEXE /app/explorer++/Explorer++.exe
...
WINEPREFIX=~/.local/share/org.winehq.proton-ge64-1/default $WINEEXE "$@"
fi
