...
if [[ $(file --mime-type -b "$1") = application/* ]]; then
    ...
    bottles_dir="$HOME/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix"    
	
	# application shortcut folder
    mkdir -p "$HOME/.local/share/applications/flatpak-wine601/$myBaseNamePrefix"
    mkdir -p "$bottles_dir"
...
shortcut" --entry)
    ...
    if [ ! $appName ]; #if no name is given use filename
       then         
       appName=$(echo "$myBaseName (6.0.1)");     
    else 
       appName=$(echo "$appName (6.0.1)");   
    fi


# try to add wineprefix for the game
mkdir -p ~/.local/share/flatpak-wine601/bottles/


###############################################
cat << EOF > ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix.sh
#!/bin/bash
export WINEPREFIX=~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix
...
# rm -rfv ~/.var/app/org.winehq.flatpak-wine/data/applications/wine*
   ...
   if [ ! -f ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix.symlinks-removed  ]; then
   
   
     rm ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/Desktop   
     rm ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/Downloads
	  rm ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Documents'
	  rm ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Music'
	  rm ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Pictures'
	  rm ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Videos' 
	  rm ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/Templates

	  #Create normal folders for the deleted symlinks
	  mkdir -p ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/Desktop
	  mkdir -p ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/Downloads
	  mkdir -p ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Documents'
	  mkdir -p ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Music'
	  mkdir -p ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Pictures'
	  mkdir -p ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/'My Videos' 
	  mkdir -p ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix/drive_c/users/\$USER/Templates

	  touch ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix.symlinks-removed 
   fi
...
cd "\$dire"
flatpak run --command=wine org.winehq.flatpak-wine601 "\$base"
exit 0;
fi
...
flatpak run --command=winetricks org.winehq.flatpak-wine601 --gui
...
prog=\$(echo \$step)
	...
	( for i in \${mydlls[*]};
	  do
    	echo \$prog
	    echo "# Installing \$i..."
	    flatpak run --command=winetricks org.winehq.flatpak-wine601 --force --unattended \$i
...
flatpak run --command=winecfg org.winehq.flatpak-wine601
elif [ "\$choice" = "Open Explorer++..." ]; then
flatpak run --command=wine org.winehq.flatpak-wine601 /app/explorer++/Explorer++.exe
elif [ "\$choice" = "Open Shell..." ]; then   
 gnome-terminal -- bash -c "flatpak run --command=bash org.winehq.flatpak-wine601"
...
shortcut" --entry)
    ...
    desktop-file-edit --set-name="\$appName" \$HOME/.local/share/applications/flatpak-wine601/"$myBaseName".desktop 
	...
	zenity --info --title="$myBaseName.desktop" --text="Shortcut renamed sucessfuly: \$appname"
     ...
     desktop-file-edit --set-icon="\$get_icon" \$HOME/.local/share/applications/flatpak-wine601/"$myBaseName".desktop 
	 update-desktop-database ~/.local/share/applications/
     ...
     DATE=\$(date +'%Y%m%d')
     mkdir -p \$HOME/flatpak-wine601-backup;
	 tar -zcvf "\$HOME/flatpak-wine601-backup/flatpak-wine601-bottle-$myBaseNamePrefix-\$DATE.tar.gz" \
			~/.local/share/flatpak-wine601/bottles/"$myBaseNamePrefix"          \
			~/.local/share/flatpak-wine601/bottles/"$myBaseNamePrefix.sh"       \
			~/.local/share/flatpak-wine601/bottles/"$myBaseNamePrefix.desktop"  \
			~/.local/share/flatpak-wine601/bottles/"$myBaseNamePrefix.icon.png" \
	     && zenity --info --title="Bottle backed up: $myBaseNamePrefix" --text="flatpak-wine601-bottle-$myBaseNamePrefix-\$DATE.tar.gz at \$HOME/flatpak-wine601-backup"



elif [ "\$choice" = "Delete Bottle: $myBaseNamePrefix" ]; then
rm -rfv ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix; 
rm -f "$HOME/.local/share/applications/flatpak-wine601/$myBaseName.desktop"
rm -f "$bottles_dir.desktop" 
rm -f "$bottles_dir.icon.png"
rm -f ~/.local/share/flatpak-wine601/bottles/"$myBaseNamePrefix.sh"
rm -f ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix.symlinks-removed
...
###############################################
chmod +x ~/.local/share/flatpak-wine601/bottles/"$myBaseNamePrefix.sh"
chmod +x ~/.local/share/flatpak-wine601/bottles/*.sh

    # Generate desktop entry specifications to be added to the application launcher.
    deskEntry="[Desktop Entry]"\\n"Exec=bash -c ~/.local/share/flatpak-wine601/bottles/$myBaseNamePrefix.sh"\\n"Name=\
...
$category;"\\n"Icon="$bottles_dir.icon.png""
    ...
    mkdir -p "$HOME/.local/share/applications/flatpak-wine601/"
    
    ln -sf "$bottles_dir".desktop  $HOME/.local/share/applications/flatpak-wine601/"$myBaseName".desktop
